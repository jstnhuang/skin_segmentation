...
void Projection::TryThresholds(cv::InputArray thermal) {
  ...
  cv::createTrackbar("Threshold", "Normalized", &slider, 5000,
                     &Projection::onTrack, this);

  cv::waitKey();
}

void Projection::onTrack(int value, void* ptr) {
  Projection* that = static_cast<Projection*>(ptr);
  cv::Mat output;
